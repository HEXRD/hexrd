...
def rot_80z_quat():
    ...
    return np.array([[val_w], [0.0], [0.0], [val_z]])
...
def test_quatOfAngleAxis():
    ...
    val = np.sqrt(2) / 2
    expected = np.array([[val], [val], [0.0], [0.0]])
...
def test_misorientation(rot_90z_quat, identity_quat):
    ...
    angle_sym, _ = rotations.misorientation(
        identity_quat, rot_90z_quat, symmetries=(d4h,)
    )
    ...
    with pytest.raises(RuntimeError):
        rotations.misorientation(
            identity_quat, identity_quat, symmetries=(1, 2, 3)
        )
...
def test_quatAverage_full_coverage(rot_90z_quat, identity_quat):
    ...
    with patch('hexrd.core.rotations.quatOfExpMap', side_effect=fixed_exp_map):
        ...
        expected_45 = np.array([[0.9238795], [0.0], [0.0], [0.3826834]])
    ...
    expected_60 = np.array([[0.8660254], [0.0], [0.0], [0.5]])
...
def test_distanceToFiber(rot_90z_quat):
    ...
    s_anti = np.array([[0.0], [0.0], [-1.0]])

    d_no_cs = rotations.distanceToFiber(
        c, s_anti, q, qsym, centrosymmetry=False
    )
