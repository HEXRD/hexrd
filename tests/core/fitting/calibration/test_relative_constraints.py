...
def test_constraints_group_init_and_reset(mock_instr):
    con = create_relative_constraints(
        RelativeConstraintsType.group, mock_instr
    )
...
def test_constraints_system_init_and_reset(mock_instr):
    con = create_relative_constraints(
        RelativeConstraintsType.system, mock_instr
    )
...
def test_enum_types():
    ...
    assert (
        RotationCenter.instrument_mean_center.value == 'InstrumentMeanCenter'
    )
...
def test_factory_types(mock_instr):
    c_none = create_relative_constraints(
        RelativeConstraintsType.none, mock_instr
    )
    assert isinstance(c_none, RelativeConstraintsNone)

    c_grp = create_relative_constraints(
        RelativeConstraintsType.group, mock_instr
    )
    assert isinstance(c_grp, RelativeConstraintsGroup)

    c_sys = create_relative_constraints(
        RelativeConstraintsType.system, mock_instr
    )
