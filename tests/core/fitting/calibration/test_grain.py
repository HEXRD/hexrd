...
@pytest.fixture
def grain_params():
    return np.array([0.1, 0.2, 0.3, 10.0, 20.0, 30.0, 1.0, 1.0, 1.0, 0.0, 0.0, 0.0])
...
def test_residual(mock_obj, calibrator):
    calibrator.data_dict = {'pick_xys': {'det1': [[10, 10, 0]]}, 'hkls': {'det1': [[1, 1, 1]]}}
    calibrator.residual()
...
def test_sxcal_obj_func_residual_mode(mock_mutil, mock_xrd, mock_gutil, mock_xfc, mock_ang, mock_instr):
    ...
    mock_gutil.matchOmegas.return_value = ([0, 1], np.array([0.15, 0.25])) # d_ome = 0.05
    mock_xfc.make_sample_rmat.return_value = np.zeros((2, 3, 3))
    mock_xfc.gvec_to_xy.return_value = np.array([[11, 11], [21, 21]]) # dx, dy = 1.0
    mock_ang.return_value = np.array([0.05, 0.05])

    result = sxcal_obj_func(grain_params, mock_instr, xyo_det, hkls_idx, np.eye(3), [0, 360], sim_only=False)

    assert result.shape == (6,)
    np.testing.assert_allclose(result[0::3], 1.0)  # X diffs
    np.testing.assert_allclose(result[1::3], 1.0)  # Y diffs
    np.testing.assert_allclose(result[2::3], 0.05) # Ome diffs

@patch('hexrd.core.fitting.calibration.grain.xfcapi')
@patch('hexrd.core.fitting.calibration.grain.grainutil')
@patch('hexrd.core.fitting.calibration.grain.xrdutil')
@patch('hexrd.core.fitting.calibration.grain.mutil')
def test_sxcal_obj_func_sim_mode(mock_mutil, mock_xrd, mock_gutil, mock_xfc, mock_instr):
    grain_params = [np.zeros(12)]
    ...
    retval = sxcal_obj_func(grain_params, mock_instr, xyo_det, hkls_idx, np.eye(3), [0, 360], sim_only=True)
...
@patch('hexrd.core.fitting.calibration.grain.xrdutil')
@patch('hexrd.core.fitting.calibration.grain.mutil')
def test_sxcal_obj_func_infeasible_warning(mock_mutil, mock_xrd, mock_gutil, mock_xfc, mock_log, mock_instr):
    mock_xfc.make_rmat_of_expmap.return_value = np.eye(3)
    mock_mutil.vecMVToSymm.return_value = np.eye(3)
    mock_mutil.unitVector.side_effect = lambda x: x
    mock_gutil.matchOmegas.return_value = ([0], np.array([0.5]))
    ...
    sxcal_obj_func([np.zeros(12)], mock_instr, xyo, hkl, np.eye(3), [0, 360], sim_only=True)
...
@patch('hexrd.core.fitting.calibration.grain.xrdutil')
@patch('hexrd.core.fitting.calibration.grain.mutil')
def test_sxcal_obj_func_with_distortion(mock_mutil, mock_xrd, mock_gutil, mock_xfc, mock_instr):
    ...
    with patch('hexrd.core.fitting.calibration.grain.angularDifference', return_value=np.array([0])):
        sxcal_obj_func([np.zeros(12)], mock_instr, xyo, hkl, np.eye(3), [0, 360], sim_only=False)

    distortion_mock.apply.assert_called()
    np.testing.assert_array_equal(distortion_mock.apply.call_args[0][0], [[10, 10]])