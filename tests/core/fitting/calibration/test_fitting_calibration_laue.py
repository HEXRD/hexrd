...
def test_laue_attributes_and_setters(laue_calibrator):
    ...
    assert (
        laue_calibrator.tth_distortion['det1'].panel
        == laue_calibrator.instr.detectors['det1']
    )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        {
            "id": "fit_fail_edge",
            "lsq": ([10, 1, 0, 1, 0.1, 0.1, 0, 0, 0], 1),
            "expect": 0,
        },
        {
            "id": "fit_fail_int",
            "lsq": ([0.1, 1, 0, 1, 5, 5, 0, 0, 0], 1),
            "expect": 0,
        },
...
def test_autopick_points(
    ...
    if not use_blob:
        mock_filt.gaussian.return_value[5, 5] = 100

    mock_blob.return_value = np.array(config.get("blob_ret", [[5, 5, 1.0]]))
    mock_lsq.return_value = config.get(
    ...
    det.clip_to_panel.return_value = (
        None,
        np.array([config.get("clip", True)]),
    )
    if config.get("dist"):
        det.distortion = MagicMock()
        det.distortion.apply_inverse.return_value = np.array(
            config.get("res_pt")
        )

    res = laue_calibrator.autopick_points(
        {'det1': np.zeros((200, 200))},
        use_blob_detection=use_blob,
        fit_peaks=fit,
    )
...
def test_laue_methods_and_obj_func(
    ...
    assert mock_obj_func.call_args[0][5][0] == 5.0
    ...
    assert len(res_sim['det1']) == 2

    meas_xy = {'det1': np.array([[99.0, 99.0]])}
    res_res = sxcal_obj_func(
        np.zeros(12),
        mock_instr,
        meas_xy,
        {'det1': np.zeros((1, 3))},
