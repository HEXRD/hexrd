...
def mock_rotations():
    with patch(
        'hexrd.core.fitting.calibration.lmfit_param_handling.calc_angles_from_beam_vec',
        return_value=(0.1, 0.2),
    ), patch(
        'hexrd.core.fitting.calibration.lmfit_param_handling.calc_beam_vec',
        return_value=np.array([0, 0, 1]),
    ), patch(
        'hexrd.core.fitting.calibration.lmfit_param_handling.angleAxisOfRotMat',
        return_value=(0.5, np.array([1, 0, 0])),
    ), patch(
        'hexrd.core.fitting.calibration.lmfit_param_handling.expMapOfQuat',
        return_value=np.array([0.1, 0.1, 0.1]),
    ), patch(
        'hexrd.core.fitting.calibration.lmfit_param_handling.make_rmat_euler',
        return_value=np.eye(3),
    ) as m_mre, patch(
        'hexrd.core.fitting.calibration.lmfit_param_handling.quatOfRotMat',
        return_value=np.array([1, 0, 0, 0]),
    ), patch(
        'hexrd.core.fitting.calibration.lmfit_param_handling.rotMatOfExpMap',
        return_value=np.eye(3),
    ) as m_rme, patch(
        'hexrd.core.fitting.calibration.lmfit_param_handling.RotMatEuler'
    ) as m_class_rme:
...
def mock_instrument():
    ...
    instr.beam_names, instr.has_multi_beam = (
        list(instr.beam_dict.keys()),
        False,
    )

    det1 = MagicMock(
        tvec=np.array([10.0, 0.0, 0.0]),
        tilt=np.zeros(3),
        rmat=np.eye(3),
        distortion=None,
        detector_type='flat',
...
def test_tilt_to_rmat_branches(mock_rotations):
    ...
    mock_rotations['rme'].assert_called()
    lph._tilt_to_rmat(np.zeros(3), ('xyz', True))
    mock_rotations['mre'].assert_called()
...
@pytest.mark.parametrize(
    "ctype, convention, checks, fails",
    [
        (
            None,
            None,
            ['instr_chi', 'det_1_tvec_x', 'det_2_radius', 'det_1_expmap_x'],
            False,
        ),
        (
            RelativeConstraintsType.group,
            None,
            ['group_1_tvec_x', 'group_1_expmap_x'],
            False,
        ),
        (
            RelativeConstraintsType.system,
            None,
            ['system_tvec_x', 'system_expmap_x'],
            False,
        ),
        (
            RelativeConstraintsType.system,
...
def test_create_instr_params(
    mock_instrument, mock_rotations, ctype, convention, checks, fails
):
    con = None
    if ctype and ctype != "invalid":
        con = MagicMock(
            type=ctype,
            params={
                'group-1': {'translation': np.zeros(3), 'tilt': np.zeros(3)},
    ...
    params = lph.create_instr_params(
        mock_instrument, relative_constraints=con, euler_convention=convention
    )
...
def test_update_instrument_scenarios(mock_instrument, mock_rotations):
    ...
    base_vars = [
        ...
        'beam_azimuth',
        'beam_polar',
        'beam_energy_correction_slope',
        'beam_energy_correction_intercept',
        'instr_chi',
        'instr_tvec_x',
        'instr_tvec_y',
        'instr_tvec_z',
        ...
        'det_2_expmap_z',
        'det_2_radius',
        'det_2_distortion_param_0',
        'det_2_distortion_param_1',
        'group_1_tvec_x',
        'group_1_tvec_y',
        'group_1_tvec_z',
        'group_1_expmap_x',
    ...
    lph.update_instrument_from_params(
        mock_instrument, params, euler_convention=None
    )
    ...
    params['group_1_tvec_x'].value = 10.0
    params['group_1_tvec_x'].vary = True
    lph.update_instrument_from_params(
        mock_instrument,
        params,
        relative_constraints=con,
    ...
    with pytest.raises(NotImplementedError):
        lph.update_instrument_from_params(
            mock_instrument, params, relative_constraints=con
        )
...
def test_update_distortion_mismatch(mock_instrument):
    ...
    with pytest.raises(RuntimeError, match="expects 2 params"):
        lph.update_unconstrained_detector_parameters(
            mock_instrument, params, None
        )
...
def test_fix_detector_y(ctype, prefixes, mock_instrument):
    params = lmfit.Parameters()
    for p in ['det_1', 'det_2', 'group_1', 'system']:
        params.add(f'{p}_tvec_y', 0.0, vary=True)
    con = MagicMock(type=ctype) if ctype else None
    lph.fix_detector_y(mock_instrument, params, relative_constraints=con)
    for p in prefixes:
        assert params[f'{p}_tvec_y'].vary is False
...
def test_engineering_constraints_tardis(y2, y4, expect):
    ...
    p2 = lmfit.Parameters()
    lph.add_engineering_constraints(p2, 'Unknown')
    assert len(p2) == 0
...
def test_material_grain_and_utils(mock_material, mock_instrument):
    ...
    assert (
        lph.create_grain_params("g1", np.zeros(12))[0][0] == 'g1_grain_param_0'
    )

    lph.validate_params_list([('a', 1), ('b', 2)])
    with pytest.raises(lph.LmfitValidationException):
        lph.validate_params_list([('a', 1), ('a', 2)])
    ...
    assert (
        lph.create_tth_parameters(mock_instrument, meas)[0][0] == 'DS_ring_0'
    )
...
def test_detector_angles_get_set(mock_instrument, mock_rotations):
    ...
    np.testing.assert_array_equal(
        lph.detector_angles_euler(det, None), det.tilt
    )
    res = lph.detector_angles_euler(det, ('xyz', True))
    np.testing.assert_allclose(res, np.degrees([0.1, 0.2, 0.3]))

    params = lmfit.Parameters()
    params.add_many(
        ('det_1_expmap_x', 1), ('det_1_expmap_y', 2), ('det_1_expmap_z', 3)
    )
    ...
    with patch(
        'hexrd.core.fitting.calibration.lmfit_param_handling.expMapOfQuat',
        return_value=np.array([5, 5, 5]),
    ):
        params.add_many(
            ('det_1_euler_x', 1), ('det_1_euler_y', 2), ('det_1_euler_z', 3)
        )
