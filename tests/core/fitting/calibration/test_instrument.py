...
def instrument_calibrator():
    ...
    with patch(
        'hexrd.core.fitting.calibration.instrument.create_instr_params',
        return_value=[],
    ), patch(
        'hexrd.core.fitting.calibration.instrument.create_relative_constraints'
    ) as mock_rel_create:

        mock_rel_create.return_value = MagicMock(
            spec=RelativeConstraints, type=RelativeConstraintsType.none
        )
...
def test_make_lmfit_params(instrument_calibrator):
    ...
    with patch(
        'hexrd.core.fitting.calibration.instrument.create_instr_params',
        return_value=[p_tuple],
    ) as mock_create, patch(
        'hexrd.core.fitting.calibration.instrument.validate_params_list'
    ) as mock_val, patch(
        'hexrd.core.fitting.calibration.instrument.add_engineering_constraints'
    ) as mock_add:

        instrument_calibrator.mock_calib.create_lmfit_params.return_value = [
            ('p2', 2.0, True, None, None, None)
        ]
...
def test_update_and_reset_logic(instrument_calibrator):
    with patch(
        'hexrd.core.fitting.calibration.instrument.update_instrument_from_params'
    ) as mock_upd:
    ...
    with patch.object(
        instrument_calibrator, 'make_lmfit_params', return_value="new_params"
    ):
...
@pytest.mark.parametrize(
    "method, call_check",
    [('least_squares', 'least_squares'), ('nelder-mead', 'scalar_minimize')],
)
...
def test_minimizer_function_and_residual(instrument_calibrator):
    ...
    with patch.object(
        instrument_calibrator, 'update_all_from_params'
    ) as mock_upd:
...
def test_engineering_constraints_logic(instrument_calibrator):
    ...
    with patch(
        'hexrd.core.fitting.calibration.instrument.map',
        create=True,
        return_value=['valid1'],
    ):
...
def test_relative_constraints_logic(instrument_calibrator):
    instrument_calibrator._relative_constraints.type = (
        RelativeConstraintsType.none
    )
    assert (
        instrument_calibrator.relative_constraints_type
        == RelativeConstraintsType.none
    )
    ...
    with patch(
        'hexrd.core.fitting.calibration.instrument.RelativeConstraintsType',
        MockRelType,
    ), patch(
        'hexrd.core.fitting.calibration.instrument.create_relative_constraints'
    ) as mock_create, patch.object(
        instrument_calibrator, 'make_lmfit_params'
    ):

        mock_create.return_value = MagicMock(type=MockRelType.instrument)
        instrument_calibrator.relative_constraints_type = (
            MockRelType.instrument
        )

        mock_create.assert_called()
        assert (
            instrument_calibrator.relative_constraints.type
            == MockRelType.instrument
        )
...
@pytest.mark.parametrize(
    "residuals, expected_log",
    [
        (
            [np.array([1.0]), np.array([0.707])],
            'OPTIMIZATION SUCCESSFUL',
        ),
        (
            [np.array([1.0]), np.array([1.0])],
            'no improvement in residual',
        ),
    ],
)
...
def test_run_calibration(instrument_calibrator, residuals, expected_log):
    ...
    with patch(
        'hexrd.core.fitting.calibration.instrument.logger'
    ) as mock_logger, patch.object(
        instrument_calibrator, 'minimize', return_value=mock_res_obj
    ), patch.object(
        instrument_calibrator, 'update_all_from_params'
    ) as mock_upd, patch.object(
        instrument_calibrator, 'residual', side_effect=residuals
    ):
