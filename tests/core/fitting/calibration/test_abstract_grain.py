...
@pytest.mark.parametrize(
    "start_val, force_val, min_val, max_val, expected_min, expected_max",
    [
...
@patch(
    'hexrd.core.fitting.calibration.abstract_grain.rename_to_avoid_collision'
)
...
@pytest.mark.parametrize(
    "method_name, param_indices, expected_vals",
    [
...
def test_fix_params_methods(
    calibrator, method_name, param_indices, expected_vals
):
    names = [f'p{i}' for i in range(12)]
    calibrator.param_names = names
    params = lmfit.Parameters()
    for n in names:
        params.add(n, value=999, vary=True)

    with patch(
        'hexrd.core.fitting.calibration.abstract_grain.cnst'
    ) as mock_cnst:
...
def test_grain_params_euler_io(mock_rme, mock_xfcapi, calibrator):
    ...
    with patch(
        'hexrd.core.fitting.calibration.abstract_grain.angleAxisOfRotMat'
    ) as mock_aa:
...
def test_grain_params_euler_no_convention(calibrator):
    ...
    np.testing.assert_array_equal(
        calibrator.grain_params_euler, calibrator.grain_params
    )


@patch(
    'hexrd.core.fitting.calibration.abstract_grain.hkl_to_str',
    side_effect=lambda x: f"{x[0]}",
)
@patch(
    'hexrd.core.fitting.calibration.abstract_grain.str_to_hkl',
    side_effect=lambda x: [int(x)],
)
...
def test_abstract_enforcement():
    class Incomplete(AbstractGrainCalibrator):
        pass
