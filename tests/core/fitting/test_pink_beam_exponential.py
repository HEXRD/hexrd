...
from hexrd.core.fitting.spectrum import (
    pink_beam_exponential as spectrum_pink_beam_exponential,
    _initial_guess,
...
class TestPeakFunctions:
    ...
    def test_gaussian_exponential(self):
        p = np.array([1.0, 10.0, 1.5, 0.1])
        x = np.linspace(8, 12, 200)
        g = pf._gaussian_exponential(p, x)
        assert g.shape == x.shape
        assert np.all(np.isfinite(g))
        assert abs(x[np.argmax(g)] - 10.0) < 1.0

    def test_gaussian_exponential_far_from_center(self):
        p = np.array([1.0, 10.0, 1.5, 0.1])
        g = pf._gaussian_exponential(p, np.array([100.0, 200.0, -100.0]))
        assert np.allclose(g, 0.0)

    def test_lorentzian_exponential(self):
        p = np.array([1.0, 10.0, 1.5, 0.1])
        x = np.linspace(8, 12, 200)
        l_val = pf._lorentzian_exponential(p, x)
        assert l_val.shape == x.shape
        assert np.all(np.isfinite(l_val))

    def test_pink_beam_exponential_no_bg(self):
        p = np.array([100.0, 10.0, 1.58, -1.35, 0.36, 0.1, 0.1])
        x = np.linspace(8, 12, 200)
        result = pf._pink_beam_exponential_no_bg(p, x)
    ...
    def test_amplitude_scaling(self):
        ...
        r1 = pf._pink_beam_exponential_no_bg(p1, x)
        r2 = pf._pink_beam_exponential_no_bg(p2, x)
    ...
    def test_zero_tau_returns_zeros(self):
        ...
        assert np.allclose(pf._pink_beam_exponential_no_bg(p, x), 0.0)
    ...
    def test_background_added(self):
        ...
        np.testing.assert_allclose(
            pf.pink_beam_exponential(p_full, x)
            - pf._pink_beam_exponential_no_bg(p_no_bg, x),
            bkg_c0 + bkg_c1 * x,
            rtol=1e-12,
        )
    ...
    def test_lmfit_matches_no_bg(self):
        ...
        lmfit_result = pf.pink_beam_exponential_lmfit(
            x, A, x0, tau0, tau1, tau2, fwhm_g, fwhm_l
        )
        p = np.array([A, x0, tau0, tau1, tau2, fwhm_g, fwhm_l])
        np.testing.assert_allclose(
            lmfit_result, pf._pink_beam_exponential_no_bg(p, x), rtol=1e-10
        )
    ...
    def test_mpeak_single(self):
        ...
        p = np.array([100.0, 10.0, 1.58, -1.35, 0.36, 0.1, 0.1])
        np.testing.assert_allclose(
            pf._mpeak_1d_no_bg(p, x, 'pink_beam_exponential', 1),
            pf._pink_beam_exponential_no_bg(p, x),
            rtol=1e-12,
        )
    ...
    def test_mpeak_two_peaks(self):
        ...
        result = pf._mpeak_1d_no_bg(np.hstack([p1, p2]), x, 'pink_beam_exponential', 2)
        expected = pf._pink_beam_exponential_no_bg(
            p1, x
        ) + pf._pink_beam_exponential_no_bg(p2, x)
...
class TestFitpeak:
    ...
    def test_estimate_pk_parms(self, mock_curve_fit, mock_snip):
        ...
        p = estimate_pk_parms_1d(x, y, 'pink_beam_exponential')
        assert len(p) == 9
    ...
    def test_fit_pk_parms(self, mock_lsq):
        ...
        fit_pk_parms_1d(np.zeros(9), x, np.ones_like(x), 'pink_beam_exponential')
        assert mock_lsq.called

    def test_fit_pk_obj_residual(self):
        x = np.linspace(8, 12, 101)
        p = np.array([100.0, 10.0, 1.58, -1.35, 0.36, 0.1, 0.1, 0.5, 0.02])
        f0 = pf.pink_beam_exponential(p, x)
        resd = fit_pk_obj_1d(p, x, f0, 'pink_beam_exponential')
    ...
    def test_fit_pk_obj_bounded(self, mock_pkfuncs):
        x = np.linspace(8, 12, 101)
        f0 = np.ones_like(x)
        mock_pkfuncs.pink_beam_exponential.return_value = f0
        res = fit_pk_obj_1d_bnded(
            [10.0], x, f0, 'pink_beam_exponential', 1.0, [0.0], [5.0]
        )
        assert res[-1] == 5.0
    ...
    def test_estimate_mpk_parms(self, mock_cf, mock_snip):
        ...
        p0, bnds = estimate_mpk_parms_1d(
            [10.0, 12.0], x, np.ones_like(x), 'pink_beam_exponential', bgtype='linear'
        )
        # 2 peaks * 7 shape params + 2 linear bg params
        assert len(p0) == 16
        assert len(bnds[0]) == 16
        assert len(bnds[1]) == 16

    def test_integrated_intensities(self):
        x = np.linspace(8, 12, 200)
        p = np.array([100.0, 10.0, 1.58, -1.35, 0.36, 0.1, 0.1])
        ints = calc_pk_integrated_intensities(p, x, 'pink_beam_exponential', 1)
        assert len(ints) == 1
        assert ints[0] > 0
...
class TestSpectrum:
    def test_wrapper_matches_lmfit(self):
        ...
        np.testing.assert_allclose(
            spectrum_pink_beam_exponential(x, *args),
            pf.pink_beam_exponential_lmfit(x, *args),
            rtol=1e-12,
        )

    def test_composite_model(self):
        model = _build_composite_model(
            npeaks=2, pktype='pink_beam_exponential', bgtype='linear'
        )
        for i in range(2):
    ...
    def test_initial_guess_shape(self):
        ...
        y = (
            pf.pink_beam_exponential_lmfit(x, 5.0, 10.0, 1.58, -1.35, 0.36, 0.1, 0.1)
            + pf.pink_beam_exponential_lmfit(x, 3.0, 12.0, 1.58, -1.35, 0.36, 0.1, 0.1)
            + 0.2
            + 0.01 * x
        )
        p0 = _initial_guess(
            ...
            x,
            y,
            pktype='pink_beam_exponential',
            bgtype='linear',
            fwhm_guess=0.1,
        )
        n_pk = num_func_params['pink_beam_exponential']
        n_bg = num_func_params['linear']
        assert p0.size == len(centers) * n_pk + n_bg

    def test_spectrum_model_fit_runs(self):
        # The exponential (tau-based) shape has a near-degenerate landscape for
        # an isolated single peak (the center trades off against the exponential
        # tail), so we only smoke-test that the full SpectrumModel pipeline runs
        # and returns a well-formed result rather than asserting tight center
        # recovery.
        ...
        fwhm_g = fwhm_l = 0.1
        y = (
            pf.pink_beam_exponential_lmfit(
        ...
        assert sm.pktype == 'pink_beam_exponential'
        assert any('tau' in n for n in sm.peak_params.keys())
        res = sm.fit()
        assert hasattr(res, 'params') and hasattr(res, 'success')
        assert np.all(np.isfinite(res.best_fit))

    def test_tau_constraints_shared(self):
        x = np.linspace(8, 14, 301)
        cen1, cen2 = 10.0, 12.0
        tau0, tau1, tau2 = 1.58, -1.35, 0.36
        y = (
            pf.pink_beam_exponential_lmfit(x, 5.0, cen1, tau0, tau1, tau2, 0.1, 0.1)
            + pf.pink_beam_exponential_lmfit(x, 3.0, cen2, tau0, tau1, tau2, 0.1, 0.1)
            + 0.01
        )
        data = np.vstack([x, y]).T
        sm = SpectrumModel(
        ...
        res = sm.fit()
        if res.success:
            # tau is held fixed and tied across peaks
            for name in ('tau0', 'tau1', 'tau2'):
                assert np.isclose(
                    res.params[f'pk1_{name}'].value,
                    res.params[f'pk0_{name}'].value,
                    rtol=1e-10,
                )
...
class TestFitRing:
    def test_successful_fit(self):
        ...
        tau0, tau1, tau2 = 1.58, -1.35, 0.36
        y = (
            pf.pink_beam_exponential_lmfit(x, 10.0, cen, tau0, tau1, tau2, 0.1, 0.1)
            + 0.01
        )
        spectrum_kwargs = {
            'pktype': 'pink_beam_exponential',
