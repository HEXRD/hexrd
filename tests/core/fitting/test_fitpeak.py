...
def test_estimate_pk_parms_1d_logic(mock_curve_fit, mock_snip, x_1d, y_gauss_1d):
...
def test_estimate_pk_parms_1d_edge_cases(mock_curve_fit, mock_snip):
    ...
    y_start = np.zeros(10); y_start[0] = 10
    fitpeak.estimate_pk_parms_1d(x, y_start, 'gaussian')
    
    y_end = np.zeros(10); y_end[-1] = 10
...
@pytest.mark.parametrize("pktype", [
    'gaussian', 'lorentzian', 'pvoigt', 'split_pvoigt', 'tanh_stepdown', 'dcs_pinkbeam'
])
...
def test_fit_pk_obj_1d_coverage(mock_pkfuncs, pktype, x_1d):
    ...
    f0 = np.ones_like(x_1d) # Non-zero to avoid division errors in pinkbeam
    p = [1, 2, 3]
    
    # Mock the return of the specific function being called
    # We cheat and make them all return f0+1 so residual is 1
    func_map = {
        'gaussian': 'gaussian1d',
    ...
    if pktype == 'dcs_pinkbeam':
        ...
        target_func.return_value = f0_nan + 1 # return anything
...
def test_fit_pk_obj_1d_bnded_coverage(mock_pkfuncs, pktype, x_1d):
    ...
    p = [10.0]
    weight = 1.0
    lb, ub = [0.0], [5.0] # p=10 violates upper bound
    ...
    lb_none = [None]
    res_none = fitpeak.fit_pk_obj_1d_bnded(p, x_1d, f0, pktype, weight, lb_none, ub)
    assert res_none[-1] == 0.0 # Should be 0 initialized, loop skips
...
def test_fit_mpk_parms_1d(mock_lsq, x_1d):
    ...
    fitpeak.fit_mpk_parms_1d([1, 2], x_1d, np.zeros_like(x_1d), 'gaussian', 1, bnds=([0], [1]))
...
def test_estimate_mpk_parms_1d(mock_cf, mock_snip, x_1d, y_gauss_1d):
    ...
    p0, _ = fitpeak.estimate_mpk_parms_1d(pk_pos, x_1d, y_gauss_1d, 'gaussian', bgtype='linear')
    assert len(p0) == 5
    
    p0, _ = fitpeak.estimate_mpk_parms_1d(pk_pos, x_1d, y_gauss_1d, 'pvoigt', bgtype='constant')
    assert len(p0) == 5
    
    p0, _ = fitpeak.estimate_mpk_parms_1d(pk_pos, x_1d, y_gauss_1d, 'split_pvoigt', bgtype='quadratic')
    assert len(p0) == 9
    
    p0, _ = fitpeak.estimate_mpk_parms_1d(pk_pos, x_1d, y_gauss_1d, 'pink_beam_dcs', bgtype='cubic')
    assert len(p0) > 4
...
@pytest.mark.parametrize("pktype", ['gaussian', 'gaussian_rot', 'split_pvoigt_rot'])
@patch('hexrd.core.fitting.fitpeak.optimize.leastsq')
...
def test_fit_pk_parms_2d_nan_check(xy_2d, z_gauss_2d):
    with patch('hexrd.core.fitting.fitpeak.optimize.leastsq') as m_lsq:
        ...
        res = fitpeak.fit_pk_parms_2d([1], xy_2d[0], xy_2d[1], z_gauss_2d, 'gaussian')
        assert res == [1]
...
def test_direct_pk_analysis(mock_plt, x_1d, y_gauss_1d):
    ...
    y_edge = np.zeros_like(x_1d); y_edge[-1] = 100.0
