...
def test_quartic_quintic_and_varargs():
    ...
    c5 = -0.0005
    assert np.allclose(
        quartic_bkg(x, c0, c1, c2, c3, c4),
        Cheb((c0, c1, c2, c3, c4), domain=(min(x), max(x)))(x),
    )
    assert np.allclose(
        quintic_bkg(x, c0, c1, c2, c3, c4, c5),
        Cheb((c0, c1, c2, c3, c4, c5), domain=(min(x), max(x)))(x),
    )
    ...
    assert np.allclose(
        chebyshev_bkg(x3, *vals[:3]), quadratic_bkg(x3, *vals[:3])
    )
    assert np.allclose(
        chebyshev_bkg(x3, *vals), Cheb(vals, domain=(min(x3), max(x3)))(x3)
    )
...
def test_amplitude_guess_centered_peak():
    ...
    assert _amplitude_guess(x, cen, y, fwhm) == pytest.approx(
        np.max(y), rel=1e-6
    )
...
def test_initial_guess_pk_and_bg_slicing_and_constant_bg():
    ...
    fwhm_guess = 0.05
    y = (
        gaussian_1d(x, amp1, centers[0], fwhm_guess)
        + gaussian_1d(x, amp2, centers[1], fwhm_guess)
        + (0.2 + 0.05 * x)
    )
    ...
    p0_const = _initial_guess(
        centers,
        x,
        y_const,
        pktype="gaussian",
        bgtype="constant",
        fwhm_guess=[fwhm_guess, fwhm_guess],
    )
...
def test_multi_peak_prefixing_and_output_behavior():
    model = _build_composite_model(
        npeaks=3, pktype='gaussian', bgtype='constant'
    )
    for i in range(3):
...
def test_spectrummodel_gaussian_fit_recovers_amp_and_center():
    ...
    y = gaussian_1d(x, amp_true, cen_true, fwhm_true) + linear_bkg(
        x, c0_true, c1_true
    )
    data = np.vstack([x, y]).T
    sm = SpectrumModel(
        data, [cen_true], pktype="gaussian", bgtype="linear", fwhm_init=0.1
    )
    assert (
        sm.pktype == "gaussian" and sm.bgtype == "linear" and sm.num_peaks == 1
    )
    assert "pk0_amp" in sm.params and "c0" in sm.params and "c1" in sm.params
    res = sm.fit()
    assert res.success
    assert res.params["pk0_amp"].value == pytest.approx(
        amp_true, rel=0.05, abs=0.2
    )
    assert res.params["pk0_cen"].value == pytest.approx(
        cen_true, rel=1e-3, abs=0.05
    )
...
def test_spectrummodel_split_pvoigt_constructor_and_fit_runs():
    ...
    data = np.vstack([x, y]).T
    sm = SpectrumModel(
        data, centers, pktype="split_pvoigt", bgtype="constant", fwhm_init=0.1
    )
...
def test_spectrummodel_background_params_and_none_fwhm_guess():
    ...
    sm = s.SpectrumModel(
        data, centers, pktype="gaussian", bgtype="linear", fwhm_init=None
    )
    ...
    p0 = _initial_guess(
        centers, x, y_simple, pktype="gaussian", bgtype="linear"
    )
