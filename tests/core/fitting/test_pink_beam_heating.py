...
class TestPeakFunctions:
    ...
    def test_pink_beam_heating_no_bg(self):
        p = np.array([100.0, 10.0, 0.1, 0.1, 0.1, 0.1])
        x = np.linspace(8, 12, 200)
    ...
    def test_amplitude_scaling(self):
        x = np.linspace(8, 12, 200)
        p1 = np.array([100.0, 10.0, 0.1, 0.1, 0.1, 0.1])
        p2 = np.array([200.0, 10.0, 0.1, 0.1, 0.1, 0.1])
    ...
    def test_lmfit_matches_no_bg(self):
        ...
        lmfit_result = pf.pink_beam_heating_lmfit(
            x, A, x0, sigma0, sigma1, fwhm_g, fwhm_l
        )
        p = np.array([A, x0, sigma0, sigma1, fwhm_g, fwhm_l])
        np.testing.assert_allclose(
    ...
    def test_mpeak_two_peaks(self):
        ...
        p1 = np.array([100.0, 10.0, 0.1, 0.1, 0.1, 0.1])
        p2 = np.array([80.0, 12.0, 0.1, 0.1, 0.1, 0.1])
        result = pf._mpeak_1d_no_bg(np.hstack([p1, p2]), x, 'pink_beam_heating', 2)
        expected = pf._pink_beam_heating_no_bg(p1, x) + pf._pink_beam_heating_no_bg(
            p2, x
        )
...
class TestFitpeak:
    ...
    def test_estimate_pk_parms(self, mock_curve_fit, mock_snip):
        ...
        assert len(p) == 8
        assert p[2] == sigma0_DFLT
        assert p[3] == sigma1_DFLT

    @patch('hexrd.core.fitting.fitpeak.optimize.least_squares')
    def test_fit_pk_parms(self, mock_lsq):
        x = np.linspace(8, 12, 101)
        mock_lsq.return_value = {'x': np.zeros(8), 'success': True}
        fit_pk_parms_1d(np.zeros(8), x, np.ones_like(x), 'pink_beam_heating')
        assert mock_lsq.called

    def test_fit_pk_obj_residual(self):
        x = np.linspace(8, 12, 101)
        p = np.array([100.0, 10.0, 0.1, 0.1, 0.1, 0.1, 0.5, 0.02])
    ...
    def test_estimate_mpk_parms(self, mock_cf, mock_snip):
        ...
        # 2 peaks * 6 shape params + 2 linear bg params
        assert len(p0) == 14
        assert len(bnds[0]) == 14
        assert len(bnds[1]) == 14
...
class TestSpectrum:
    def test_wrapper_matches_lmfit(self):
        x = np.linspace(8, 12, 200)
        args = (100.0, 10.0, 0.1, 0.1, 0.1, 0.1)
        np.testing.assert_allclose(
    ...
    def test_composite_model(self):
        ...
        for i in range(2):
            for k in ['amp', 'cen', 'sigma0', 'sigma1', 'fwhm_g', 'fwhm_l']:
        ...
        for i in range(2):
            ...
            params[f'pk{i}_sigma0'].set(value=0.1)
            params[f'pk{i}_sigma1'].set(value=0.1)
    ...
    def test_initial_guess_shape(self):
        ...
        y = (
            pf.pink_beam_heating_lmfit(x, 5.0, 10.0, 0.1, 0.1, 0.1, 0.1)
            + pf.pink_beam_heating_lmfit(x, 3.0, 12.0, 0.1, 0.1, 0.1, 0.1)
            + 0.2
            + 0.01 * x
        )
        p0 = _initial_guess(
    ...
    def test_spectrum_model_fit_recovers_center(self):
        ...
        fwhm_g = fwhm_l = 0.1
        y = (
            pf.pink_beam_heating_lmfit(x, amp, cen, sigma0, sigma1, fwhm_g, fwhm_l)
            + 0.01
        )
        ...
        assert any('sigma' in n for n in sm.peak_params.keys())
        res = sm.fit()
        assert hasattr(res, 'params') and hasattr(res, 'success')
        if res.success:
            assert abs(res.params['pk0_cen'].value - cen) < 0.5

    def test_sigma_constraints_shared(self):
        x = np.linspace(8, 14, 301)
        cen1, cen2 = 10.0, 12.0
        sigma0, sigma1 = 0.1, 0.1
        y = (
            pf.pink_beam_heating_lmfit(x, 5.0, cen1, sigma0, sigma1, 0.1, 0.1)
            + pf.pink_beam_heating_lmfit(x, 3.0, cen2, sigma0, sigma1, 0.1, 0.1)
            + 0.01
        )
        data = np.vstack([x, y]).T
        sm = SpectrumModel(
        ...
        res = sm.fit()
        if res.success:
            # sigma is held fixed and tied across peaks
            assert np.isclose(
                res.params['pk1_sigma0'].value,
                res.params['pk0_sigma0'].value,
                rtol=1e-10,
            )
            assert np.isclose(
                res.params['pk1_sigma1'].value,
                res.params['pk0_sigma1'].value,
                rtol=1e-10,
            )
...
class TestFitRing:
    def test_successful_fit(self):
        ...
        cen = 10.0
        sigma0, sigma1 = 0.1, 0.1
        y = pf.pink_beam_heating_lmfit(x, 10.0, cen, sigma0, sigma1, 0.1, 0.1) + 0.01
        spectrum_kwargs = {
            'pktype': 'pink_beam_heating',
            'bgtype': 'linear',
            'fwhm_init': 0.1,
