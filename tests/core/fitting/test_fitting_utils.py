...
def params():
    p = Parameters()
    p.add_many(('cen_1', 10.0, True), ('cen_2', 20.0, True), ('fwhm_1', 0.1, True), 
               ('mixing_1', 0.5, True), ('other', 1.0, True))
    return p
...
def test_param_helpers(params):
    ...
    with pytest.raises(RuntimeWarning): utils._set_refinement_by_name(params, 'bad')
...
def test_equality_constraints(params):
    ...
    with pytest.raises(RuntimeWarning): utils._set_equality_constraints(params, 'bad')
...
def test_fit_ring(mock_sm_cls):
    data = (np.array([1., 2., 3.]), np.array([10., 50., 10.]), [2.0])
    mock_res = MagicMock()
