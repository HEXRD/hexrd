...
def env():
    ...
    instr = MagicMock()
    instr.beam_vector, instr.eta_vector = np.array([0, 0, 1]), np.array(
        [1, 0, 0]
    )
...
def test_fitGrain(mock_lsq, env):
    ...
    with pytest.raises(RuntimeError):
        fitGrain(
            env['g'], env['inst'], {}, env['B'], env['wl'], omePeriod=[0, 360]
        )
...
def test_matchOmegas(mock_map, mock_os, case, env):
    ...
    _, res = matchOmegas(
        xyo, env['hkl'], 0.0, np.eye(3), env['B'], 0.5, omePeriod=ome_p
    )
...
def test_objFunc_evaluation(mock_g2xy, mock_ext, mock_match, mode, env):
    ...
    if mode == "distortion":
        ...
        env['inst'].detectors['det1'].distortion.apply.return_value = np.array(
            [[5.0, 5.0]]
        )

    kwargs = {'simOnly': mode.startswith("sim")}
    if mode == "sim_dict" or mode == "scalar_chi":
        kwargs['return_value_flag'] = 2
    elif mode == "scalar_sum":
        kwargs['return_value_flag'] = 1
...
def test_objFunc_mixed_empty(mock_g2xy, mock_ext, mock_mat, env):
    mock_ext.return_value = (np.eye(3), np.zeros(3), 0.0, np.zeros(3))
    mock_g2xy.return_value = np.array([[0.0, 0.0]])
    mock_mat.return_value = (None, np.array([0.0]))
