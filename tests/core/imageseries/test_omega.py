...
import numpy as np
from hexrd.core.imageseries.omega import OmegaImageSeries, OmegaWedges, OmegaSeriesError

# --- OmegaWedges Tests ---

def test_omegawedges_basic():
    """Test adding wedges, frame counting, and array generation."""
    ow = OmegaWedges(20)
...
def test_omegawedges_error():
    """Test error when generated frames don't match expected count."""
    ow = OmegaWedges(20)
    ow.addwedge(0, 10, 5) 
...
def test_omegawedges_save(mock_save):
    """Test save_omegas calls numpy save."""
    ow = OmegaWedges(10)
    ow.addwedge(0, 10, 10)
...
def test_omegaseries_init_error_no_meta(mock_ims):
    """Test init failure when omega key missing."""
...
def test_omegaseries_init_error_mismatch(mock_ims):
    """Test init failure when omega array shape mismatch."""
...
def test_omegaseries_init_valid(mock_ims):
    """Test valid initialization with single contiguous wedge."""
    om = np.zeros((10, 2))
    om[:, 0] = np.arange(10)
    om[:, 1] = np.arange(1, 11)
...
def test_make_wedges_non_contiguous(mock_ims):
    """Test detection of breaks in omega sequence."""
    om = np.zeros((10, 2))
...
def test_make_wedges_delta_change(mock_ims):
    """Test detection of breaks when delta step changes."""
    om = np.zeros((10, 2))
    om[0:5, 0] = np.arange(5)
    om[0:5, 1] = np.arange(1, 6)
...
def test_make_wedges_decreasing_error(mock_ims):
    """Test error on non-increasing omega."""
    om = np.zeros((10, 2))
    om[0, 0] = 1.0
    om[0, 1] = 0.0
    mock_ims.metadata['omega'] = om
...
def test_omega_to_frame(mock_ims):
    om = np.zeros((20, 2))
    om[0:10, 0] = np.arange(10); om[0:10, 1] = np.arange(1, 11)
    om[10:20, 0] = np.arange(20, 30); om[10:20, 1] = np.arange(21, 31)
...
def test_omegarange_to_frames(mock_ims):
    om = np.zeros((20, 2))
    om[0:10, 0] = np.arange(350, 360); om[0:10, 1] = np.arange(351, 361)
    om[10:20, 0] = np.arange(0, 10);   om[10:20, 1] = np.arange(1, 11)
...
def test_omegarange_gap(mock_ims):
    """Test behavior when a range crosses a non-wrapping gap."""
    om = np.zeros((20, 2))
    om[0:10, 0] = np.arange(10); om[0:10, 1] = np.arange(1, 11)
    om[10:20, 0] = np.arange(20, 30); om[10:20, 1] = np.arange(21, 31)
