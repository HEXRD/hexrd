...
def mock_npz():
    return {
        ...
        '0_row': np.array([0]),
        '0_col': np.array([0]),
        '0_data': np.array([10.0]),
        '1_row': np.array([1]),
        '1_col': np.array([1]),
        '1_data': np.array([20.0]),
        'meta_key': 'val',
    }
...
def test_init_npz(mock_load, mock_npz):
    ...
    assert (
        len(a) == 2 and a.shape == (10, 10) and a.metadata['meta_key'] == 'val'
    )


@patch('hexrd.core.imageseries.load.framecache.h5py.File')
@patch(
    'hexrd.core.imageseries.load.framecache.h5py_read_string',
...
def test_init_error():
    with pytest.raises(TypeError):
        FrameCacheImageSeriesAdapter("t", style="bad")
...
def test_yml_paths(mock_open, mock_yaml, mock_loader):
    mock_yaml.return_value = {
        'data': {'file': 'c.npz', 'nframes': 1, 'shape': [1], 'dtype': 'f4'},
        'meta': {},
    }
...
def test_internal_loaders(mock_file, mock_h5, mock_npz):
    ...
    from hexrd.core.imageseries.load.framecache import (
        _load_framecache_fch5,
        _load_framecache_npz,
    )
...
def test_fch5_errors(mock_file):
    ...
    with pytest.raises(NotImplementedError):
        FrameCacheImageSeriesAdapter("t.h5", style="fch5")
