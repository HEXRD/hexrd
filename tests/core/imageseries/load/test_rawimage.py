...
@patch('builtins.open', new_callable=mock_open)
@patch('hexrd.core.imageseries.load.rawimage.os.path.getsize')
def test_getitem_access(mock_getsize, mock_file, mock_fromfile, mock_yaml_content):
...
@patch('builtins.open', new_callable=mock_open)
@patch('hexrd.core.imageseries.load.rawimage.os.path.getsize')
def test_getitem_fancy_indexing(mock_getsize, mock_file, mock_fromfile, mock_yaml_content):
...
def test_iterator_support(mock_yaml_content):
    """Test iteration."""
    with patch('builtins.open', mock_open(read_data=yaml.dump(mock_yaml_content))) as m_open, \
         patch('hexrd.core.imageseries.load.rawimage.os.path.getsize', return_value=500), \
         patch('hexrd.core.imageseries.load.rawimage.np.fromfile', return_value=np.zeros(100)):
