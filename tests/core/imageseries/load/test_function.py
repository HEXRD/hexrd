...
def test_function_adapter_init(frame_generator):
    ...
    meta = {'pixel_size': 0.1}
    adapter = FunctionImageSeriesAdapter(None, func=frame_generator, num_frames=5, meta=meta)
...
def test_function_adapter_access(frame_generator):
    ...
    adapter = FunctionImageSeriesAdapter(None, func=frame_generator, num_frames=5)
    
    frame_2 = adapter[2]
    np.testing.assert_array_equal(frame_2, np.full((10, 10), 2.0, dtype=np.float32))
...
def test_function_adapter_iteration(frame_generator):
    ...
    adapter = FunctionImageSeriesAdapter(None, func=frame_generator, num_frames=3)
...
def test_function_adapter_missing_args():
    ...
    def dummy(i): return np.zeros((1,1))
