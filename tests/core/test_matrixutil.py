...
def test_unitVector(random_vectors):
    ...
    # Test zero vector handling (avoid div by zero)
    zeros = np.zeros((3, 2))
...
def test_nullSpace():
    # A * x = 0. 
    # Let A be [[1, 1, 1], [1, 1, 1]]. Rank is 1. Null space dim 2.
    A = np.array([[1., 1., 1.], [1., 1., 1.]])
    ns = matrixutil.nullSpace(A)
    
    # Check that A * ns.T is zero (approx)
    prod = np.dot(A, ns.T)
    np.testing.assert_allclose(prod, 0.0, atol=1e-10)

def test_nullSpace_transpose_branch():
    """Test branch where n > m (Tall Matrix)."""
    # The code recurses: nullSpace(A.T).T
    # This calculates the LEFT null space (y^T A = 0)
    A = np.array([[1., 1.], [2., 2.], [3., 3.]]) # 3x2
    ns = matrixutil.nullSpace(A)
...
def test_rankOneMatrix_errors():
    # 1. vec1 wrong shape (>2 dims)
    with pytest.raises(RuntimeError, match="input vec1 is the wrong shape"):
        matrixutil.rankOneMatrix(np.zeros((3, 3, 3)))
    
    # 2. vec2 wrong shape (>2 dims)
    # BUG NOTE: Source code checks len(vec1.shape) again in else block instead of vec2
    # This leads to np.asmatrix raising ValueError. We assert ValueError to match reality.
    with pytest.raises(ValueError):
        matrixutil.rankOneMatrix(np.zeros((3, 1)), np.zeros((3, 3, 3)))
        
    # 3. Vector count mismatch (n1 != n2)
...
def test_skew(random_vectors):
    # Pass COPY because skew modifies in-place
...
def test_skewMatrixOfVector_vectorOfSkewMatrix(skew_matrix):
    w_expected = np.array([1., 2., 3.])
    # vectorOfSkewMatrix modifies input in-place, pass COPY
...
def test_vectorOfSkewMatrix_errors():
    # Use raw strings for regex with parens
...
def test_multMatArray_errors():
    ...
    with pytest.raises(RuntimeError, match="mismatch on internal matrix dimensions"):
...
def test_findduplicatevectors_numba_coverage():
    ...
    # Check return structure: eqv[i, k] contains index of duplicate
    # Row 0 found duplicate at index 2
    assert eqv[0, 0] == 2.0
    
    # 2. EquivPM Match
    # vec: [v1, -v1]
    vec_pm = np.array([[1., -1.], [2., -2.]])
    eqv_pm = func(vec_pm, 1e-8, True)
    assert eqv_pm[0, 0] == 1.0
    
    # 3. No Match
    vec_none = np.array([[1., 2.], [2., 3.]])
...
def test_extract_ijv_numba_coverage():
    ...
    assert count == 2
    # Check first found (order depends on iteration, row-major usually)
    # (0,1) val 10. (1,0) val 5.
    assert out_v[0] == 10
    assert out_v[1] == 5
