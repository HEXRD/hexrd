...
class ConcreteDetector(Detector):
    ...
    def __init__(self, **kwargs):
        ...
        self._calibration_flags = [True]*6 # Default flags
    ...
    def physical_size(self):
        # Implementation required by abstract base
        return np.array([
            self.rows * self.pixel_size_row, 
    ...
    def clip_to_panel(self, pts, buffer=None):
        # Dummy implementation for base class testing if base doesn't provide it
        # Actually base Detector DOES provide clip_to_panel, but it relies on
        # physical_size and other properties we just added.
...
def test_init_defaults(base_detector):
    ...
    np.testing.assert_array_equal(d.tvec, [0., 0., -1000.])
    np.testing.assert_array_equal(d.tilt, [0., 0., 0.])
...
def test_pixel_coords_generation():
    ...
    # Check steps (allowing for float precision)
    # Note: Detector.pixel_coords logic might center at 0 or start at corner.
    # The key is the spacing.
...
def test_config_dict():
    ...
    # Set explicit saturation level to avoid NoneType error in source
...
def test_clip_to_panel():
    """Test point clipping logic."""
    # 100x100 pixels of 0.1 size -> 10x10 mm physical size.
    # Default origin usually center (0,0). Extent [-5, 5].
    ...
    # clip_to_panel returns (clipped_pts, mask)
    clipped, mask = d.clip_to_panel(pts)
