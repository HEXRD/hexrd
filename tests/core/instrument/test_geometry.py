...
def _misorientation_deg(tilt_a, tilt_b) -> float:
    q = Rotation.from_rotvec(tilt_a).inv() * Rotation.from_rotvec(tilt_b)
    return float(np.degrees(q.magnitude()))


def plant_motion(base: HEDMInstrument, monolith_tilt, monolith_trans,
                 res_tilts, res_disps) -> HEDMInstrument:
...
@pytest.fixture
def planted_truth(eiger_uncalibrated: HEDMInstrument):
...
# ======================================================================
def test_recovers_planted_monolith(eiger_uncalibrated, planted_truth):
...
def test_reconstruct_recovers_planted_final(eiger_uncalibrated, planted_truth):
...
# ======================================================================
@pytest.fixture
def planted_with_outliers(eiger_uncalibrated: HEDMInstrument):
...
def test_robust_recovers_monolith_despite_outliers(
        eiger_uncalibrated, planted_with_outliers):
...
def test_robust_flags_the_movers(eiger_uncalibrated, planted_with_outliers):
...
# ======================================================================
def test_real_pair_reconstructs_exactly(eiger_uncalibrated, eiger_calibrated):
...
def test_real_pair_residual_translations_are_mean_zero(
        eiger_uncalibrated, eiger_calibrated):
    d = decompose_instrument_motion(eiger_uncalibrated, eiger_calibrated)
...
# ======================================================================
def test_apply_monolith_move_copy_by_default(eiger_uncalibrated, planted_truth):
    final, _, _ = planted_truth
    d = decompose_instrument_motion(eiger_uncalibrated, final)
...
def test_apply_monolith_move_inplace(eiger_uncalibrated, planted_truth):
    final, _, _ = planted_truth
    d = decompose_instrument_motion(eiger_uncalibrated, final)
...
# ======================================================================
def test_mismatched_detectors_raises(eiger_uncalibrated):
...
def test_detector_keys_subset(eiger_uncalibrated, planted_truth):
    final, _, _ = planted_truth
...
def test_unknown_detector_key_raises(eiger_uncalibrated, planted_truth):
    final, _, _ = planted_truth
