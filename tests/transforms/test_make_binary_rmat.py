...
import numpy as np
from hexrd.core.transforms.xfcapi import make_binary_rmat
from hexrd.core.rotations import quatOfAngleAxis, rotMatOfQuat
...
def test_make_binary_rmat():
    ...
    for _ in range(100):
        ...
        assert np.all(
            (np.abs(rmat) - 1 < 1e-10) | (np.abs(rmat) < 1e-10)
        ), "It is binary"
        rmat_expected = rotMatOfQuat(quatOfAngleAxis(np.pi, np.c_[axis]))
