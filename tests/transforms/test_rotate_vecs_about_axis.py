...
def test_rotate_vecs_about_axis():
    ...
    assert np.allclose(np.linalg.norm(vecs, axis=0), np.linalg.norm(rotated, axis=0))

    # Check that the angle between the rotated vectors and the rotation axes is the same
    for i in range(n):
        assert np.allclose(np.arccos(np.dot(rotated[:, i], axes[:, i]) / np.linalg.norm(rotated[:,i])), np.arccos(np.dot(vecs[:, i], axes[:, i] / np.linalg.norm(vecs[:,i]))))
    
    # Project all of the vectors on the normal plane to the rotation axes
    vecs_proj = np.zeros((3,n))
    rotated_proj = np.zeros((3,n))
    for i in range(n):
        vecs_proj[:, i] = vecs[:, i] - np.dot(vecs[:, i], axes[:, i]) * axes[:, i]
        rotated_proj[:,i] = rotated[:, i] - np.dot(rotated[:, i], axes[:, i]) * axes[:, i]
    ...
    for i in range(n):
        # The angle between these vectors could be the rotation angle, pi - rotation angle, or other things as well
        # Just call arccos(cos(angle)) to solve this
        ang_fixed = np.arccos(np.cos(angles[i]))
        assert np.allclose(np.arccos(np.dot(vecs_proj[:, i], rotated_proj[:, i])), ang_fixed )
