...
from hexrd.core.imageseries.load.eiger_stream_v2 import EigerStreamV2ImageSeriesAdapter
...
def test_format_eiger_stream_v2(example_repo_path: Path):
    filepath = (
        example_repo_path / 'eiger/eiger_stream_v2/eiger_stream_v2_test_dataset.h5'
    )
    fmt = 'eiger-stream-v2'
...
def test_csrnpz_from_hdf5(tmp_path: Path):
    ...
    with h5py.File(temp_file, 'w') as f:
        ...
        img_group = data.create_group('0/threshold_1')
    ...
    adapter = EigerStreamV2ImageSeriesAdapter(
        temp_file, threshold_setting='threshold_1'
    )
...
def test_lz4_from_hdf5(tmp_path: Path):
    ...
    adapter = EigerStreamV2ImageSeriesAdapter(temp_file)
    decompressed_array = adapter[0]
    assert np.array_equal(original_array, decompressed_array)
