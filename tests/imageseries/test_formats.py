...
class TestFormatH5(ImageSeriesFormatTest):
    ...
    def test_fmth5(self):
        ...
        self.assertAlmostEqual(diff, 0.0, "h5 reconstruction failed")
    ...
    def test_fmth5_nparray(self):
        ...
        self.assertAlmostEqual(diff, 0.0, "h5 numpy array metadata failed")

    def test_fmth5_nocompress(self):
        """HDF5 options: no compression"""
        imageseries.write(
            self.is_a, self.h5file, self.fmt, path=self.h5path, gzip=0
        )
        is_h = imageseries.open(self.h5file, self.fmt, path=self.h5path)
    ...
    def test_fmth5_compress_err(self):
        ...
        with self.assertRaises(ValueError):
            imageseries.write(
                self.is_a, self.h5file, self.fmt, path=self.h5path, gzip=10
            )

    def test_fmth5_chunk(self):
        """HDF5 options: chunk size"""
        imageseries.write(
            self.is_a, self.h5file, self.fmt, path=self.h5path, chunk_rows=0
        )
        is_h = imageseries.open(self.h5file, self.fmt, path=self.h5path)
...
class TestFormatFrameCache(ImageSeriesFormatTest):
    ...
    def test_fmtfc(self):
        ...
        self.assertAlmostEqual(diff, 0.0, "frame-cache reconstruction failed")
    ...
    def test_fmtfc_nparray(self):
        ...
        self.assertAlmostEqual(
            diff, 0.0, "frame-cache numpy array metadata failed"
        )
