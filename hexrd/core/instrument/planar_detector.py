...
class PlanarDetector(Detector):
    ...
    def calc_filter_coating_transmission(
        ...
        secb = 1.0 / np.dot(dvecs, det_normal).reshape(self.shape)
    ...
    def beam_position(self):
        ...
        if np.logical_and(abs(b_dot_n) > ct.sqrt_epsf, np.sign(b_dot_n) == -1):
...
def _pixel_angles(
    ...
    angs, g_vec = xy_to_gvec(
        xy, rmat, ct.identity_3x3, tvec, ct.zeros_3, origin, rmat_b=rmat_b
    )
...
@memoize
def _pixel_tth_gradient(
    origin, pixel_coords, distortion, rmat, tvec, bvec, evec, rows, cols
):
    assert len(origin) == 3, "origin must have 3 elements"
    ptth, _ = _pixel_angles(
        origin, pixel_coords, distortion, rmat, tvec, bvec, evec, rows, cols
    )
...
def _generate_pixel_solid_angles(
    start_stop, rows, cols, pixel_size_row, pixel_size_col, rmat, tvec
):
    ...
    pcrd_array_full = (
        np.dot(
            np.vstack([pvx.flatten(), pvy.flatten(), np.zeros(nvtx)]).T, rmat.T
        )
        + tvec
    )
...
def _pixel_solid_angles(
    rows, cols, pixel_size_row, pixel_size_col, rmat, tvec, max_workers
):
