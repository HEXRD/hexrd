...
center displacement (mm), thresholded independently.

METHOD (consensus / averaging in pose space)
--------------------------------------------
  * monolith ROTATION    = the average of the per-panel rotation *changes*
                           Q_i = R_i^final (R_i^initial)^T, taken as scipy's
                           ``Rotation.mean()`` -- the chordal (L2/Frobenius) mean
                           on SO(3). It is the single rotation minimizing
                           sum_i w_i ||Q_i - Q||_F^2 and, for the sub-degree
                           spreads seen here, is indistinguishable from the
                           iterative geodesic (Karcher) mean.
  * monolith TRANSLATION = the displacement of the panel centroid.

You CANNOT average the exponential-map ``tilt`` vectors directly: the exp map is
a nonlinear chart on the rotation group (it wraps, is singular near pi, and is
not unique). Every average below is therefore done on rotation matrices.

The monolith rotation is applied as a RIGID rotation about the initial centroid,
so panel centers SWING with it; the residual center shifts are measured *after*
that swing. Skipping the swing would make an outer panel show a large fake
translation ~ (lever_arm * tilt) -- of order 1 mm for a tenth-of-a-degree tilt at
a 150 mm lever arm, dwarfing the real ~10 micron residuals.

POSE CONVENTION (matches hexrd)
-------------------------------
Each panel carries ``tilt`` (exponential map, a rotation vector, radians) and
``tvec`` (lab-frame translation, mm), with ``rmat = expmap(tilt)`` mapping
panel-local coords to lab coords via ``x_lab = rmat @ x_local + tvec``. hexrd's
exp map is identical to scipy's ``rotvec``, so we use
``scipy.spatial.transform.Rotation`` here, exactly as ``hexrd.core.rotations``
does.
"""
...
def _rmats_from_tilts(tilts: NDArray) -> NDArray:
    """Stack of exp-map tilts (N, 3) -> stack of rotation matrices (N, 3, 3)."""
    return Rotation.from_rotvec(np.asarray(tilts, float)).as_matrix()


def _expmaps_from_rmats(rmats: NDArray) -> NDArray:
    """Stack of rotation matrices (N, 3, 3) -> stack of exp-map tilts (N, 3)."""
...
class MonolithMove:
    ...
    translation : (3,) centroid displacement, mm. This is the interpretable
        "how far did the detector move" number -- it is pivot-independent,
        because the monolith rotation contributes zero centroid motion.
    pivot : (3,) point the monolith rotation turns about (the initial,
        weighted, panel centroid), mm.
    offset : (3,) lab-origin offset used to APPLY the move exactly:
        ``tvec_new = rmat @ tvec_old + offset``. Equals
        ``pivot - rmat @ pivot + translation``; reported mainly for application,
        not interpretation.
    angle : magnitude of ``tilt``, degrees (convenience).
    distance : magnitude of ``translation``, mm (convenience).
    """
    ...
    def angle(self) -> float:
        return float(np.degrees(np.linalg.norm(self.tilt)))
...
class ResidualMove:
    ...
    weight : consensus weight this panel carried (1.0 unless ``robust``
        down-weighted it; large movers come out small).
    """
...
# ======================================================================
def _decompose_arrays(tiltsA, tvecsA, tiltsB, tvecsB, weights=None):
    """Split the A -> B motion of N panels into a monolith move + residuals.

    All inputs are stacks indexed the same way: ``tilts*`` are (N, 3) exp-map
    vectors, ``tvecs*`` are (N, 3) lab-frame translations in mm. ``weights`` is
    an optional (N,) non-negative array applied CONSISTENTLY to both the rotation
    mean and the centroid (so the robust variant cannot bias one but not the
    other). Returns a dict of arrays.
    """
    ...
    # Monolith translation = (weighted) centroid displacement. The rotation
    # pivots about the state-A centroid; the offset maps centroid A -> centroid
    # B exactly. Same weights as the rotation mean.
    cenA = w @ tA
    cenB = w @ tB
    ...
    # Residual per-panel motion taking intermediate -> B. The residual rotation
    # is exact (QS_i = R_i^B (R_i^int)^T); the residual displacement is the pure
    # center shift, weighted-mean-zero by construction.
    QS = RB @ R_int.transpose(0, 2, 1)
    res_tilt = _expmaps_from_rmats(QS)
    res_dvec = tB - t_int

    return dict(
        QM=QM, offset=offset, tilt_M=_expmaps_from_rmats(QM[None])[0],
        trans_M=trans_M, pivot=cenA,
        res_tilt=res_tilt, res_dvec=res_dvec,
        res_angle=np.linalg.norm(res_tilt, axis=1),
        res_disp=np.linalg.norm(res_dvec, axis=1),
        weights=w * n,  # report relative weights (1.0 == nominal)
    )


def _robust_decompose_arrays(tiltsA, tvecsA, tiltsB, tvecsB,
                             n_iter=5, huber_angle=None, huber_distance=None):
    """IRLS/Huber version of :func:`_decompose_arrays`.

    A single genuinely-shifted or stuck module would otherwise bias the
    consensus and leak its motion into every panel's residual. We run a few
    iteratively-reweighted passes: panels that move little keep full weight,
    big movers get down-weighted when forming the monolith (both its rotation
    and its translation), so they stand out clearly as large residuals.
    """
    w = None
    d = None
    for _ in range(n_iter):
        d = _decompose_arrays(tiltsA, tvecsA, tiltsB, tvecsB, weights=w)
        ka = huber_angle if huber_angle else max(np.median(d['res_angle']), 1e-9)
        kt = huber_distance if huber_distance else max(np.median(d['res_disp']), 1e-9)
        r = np.hypot(d['res_angle'] / ka, d['res_disp'] / kt)
...
# Public API
# ======================================================================
def _shared_detector_keys(initial, final, detector_keys=None):
...
def decompose_instrument_motion(
    ...
    *,
    ...
    tiltsA = np.array([np.asarray(initial.detectors[k].tilt, float) for k in keys])
    tvecsA = np.array([np.asarray(initial.detectors[k].tvec, float) for k in keys])
    tiltsB = np.array([np.asarray(final.detectors[k].tilt, float) for k in keys])
    tvecsB = np.array([np.asarray(final.detectors[k].tvec, float) for k in keys])
...
def apply_monolith_move(
    ...
    *,
    ...
    for k in decomposition.detector_keys:
        ...
        det.tilt = _expmaps_from_rmats((QM @ det.rmat)[None])[0]
    return out
...
def reconstruct_final(
    ...
    *,
    ...
    for k in decomposition.detector_keys:
        ...
        QS = _rmats_from_tilts(res.tilt[None])[0]
        det.tilt = _expmaps_from_rmats((QS @ det.rmat)[None])[0]
