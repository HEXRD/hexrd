...
static Eigen::MatrixX3d makeOscillRotMat(const double chi, const Eigen::VectorXd& ome) {
  ...
  for (size_t i = 0; i < vec_size; i += batch_size) {
    ...
    for(size_t j = 0; j < batch_size && (i+j) < vec_size; ++j) {
      rots.block<3, 3>(3*(i+j), 0) <<         come_arr[j],    0,         some_arr[j],
                                       schi * some_arr[j], cchi, -schi * come_arr[j],
                                      -cchi * some_arr[j], schi,  cchi * come_arr[j];
    }
  }
  return rots;
}

// Both anglesToGvec and anglesToDvec, depending on if the last argument is true or false
const static Eigen::MatrixX3d anglesToVec(const Eigen::MatrixXd &angs,
                                    const Eigen::Vector3d &bHat_l,
...
const static Eigen::MatrixX3d anglesToGVec(const Eigen::MatrixXd &angs,
                                    const Eigen::Vector3d &bHat_l,
                                    const Eigen::Vector3d &eHat_l, double chi,
...
const static Eigen::MatrixX3d anglesToDVec(const Eigen::MatrixXd &angs,
                                    const Eigen::Vector3d &bHat_l,
                                    const Eigen::Vector3d &eHat_l, double chi,
                                    const Eigen::Matrix3d &rMat_c) noexcept {
...
const static Eigen::Vector2d gvecToDetectorXYOne(const Eigen::Vector3d& gVec_c, const Eigen::Matrix3d& rMat_d,
...
const static Eigen::MatrixXd gvecToDetectorXY(const Eigen::MatrixXd& gVec_c, const Eigen::Matrix3d& rMat_d,
...
const static Eigen::Vector2d gvecToDetectorXYOneSimple(const Eigen::Vector3d& gVec_l, const Eigen::Matrix3d& rMat_d,
                                    const Eigen::Vector3d& tVec_d,
                                    const Eigen::Vector3d& bHat_l, const Eigen::Vector3d& nVec_l,
                                    double num, const Eigen::Vector3d& P0_l)
{
  double bDot = -bHat_l.dot(gVec_l);
...
const static Eigen::MatrixXd gvecToDetectorXYFromAngles(const double chi, const Eigen::VectorXd omes,
                                 const Eigen::MatrixXd& gVec_c, const Eigen::Matrix3d& rMat_d,
                                 const Eigen::Matrix3d& rMat_c,
                                 const Eigen::Vector3d& tVec_d, const Eigen::Vector3d& tVec_s,
                                 const Eigen::Vector3d& tVec_c, const Eigen::Vector3d& beamVec)
{
...
const static Eigen::MatrixX2d anglesToGvecToDetectorXYFromAngles(const double chi, const Eigen::MatrixXd omes,
                                 ...
                                 const Eigen::Vector3d& tVec_c, const Eigen::Vector3d& beamVec)
{
  ...
  const Eigen::MatrixX3d gVec_c = anglesToGVec(omes, beamVec, {1, 0, 0}, chi, rMat_c).rowwise().normalized();
