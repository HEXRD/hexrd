...
def angles_to_dvec(
    ...
    return transforms.anglesToDVec(angs, beam_vec, eta_vec, chi, rmat_c)
...
def make_sample_rmat(chi: float, ome: Union[float, np.ndarray]) -> np.ndarray:
    ...
    if ome is ome_array:
        ...
        result = transforms.makeOscillRotMat(chi, ome_array).reshape(
            len(ome_array), 3, 3
        )
    else:
...
def make_rmat_of_expmap(exp_map: np.ndarray) -> np.ndarray:
    ...
    return transforms.make_rot_mat_of_exp_map(arg)
...
def make_binary_rmat(axis: np.ndarray) -> np.ndarray:
    ...
    return transforms.make_binary_rot_mat(arg)
