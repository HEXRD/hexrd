...
class AbstractWPPF(ABC):
    ...
    def _set_amorphous_params_vals_to_class(self, params: lmfit.Parameters):
        ...
        for key in self.amorphous_model.scale:
            ...
            if self.amorphous_model.model_type in (
                "split_gaussian",
                "split_pv",
            ):
            ...
            if self.amorphous_model.model_type == "split_gaussian":
                ...
                fwhm[key] = np.array([fwhm_g_l, fwhm_l_l, fwhm_g_r, fwhm_l_r])
        ...
        elif self.amorphous_model.model_type in ["split_gaussian", "split_pv"]:
    ...
    def DOC(self):
        ...
        return 1.0 - amorphous_area / self.total_area
...
class LeBail(AbstractWPPF):
    ...
    def computespectrum(self):
        ...
        for iph, p in enumerate(self.phases):
            for k, l in self.phases.wavelength.items():
                ...
                if self.phases[p].sf_alpha is None:
                    ...
                    sf_shift = (
                        alpha
                        * np.tan(np.radians(0.5 * self.tth[p][k]))
        ...
        errvec, self.Rwp, self.Rwpb, self.gofF, self.gofFb = calc_rwp(
    ...
    def RefineCycle(self, print_to_screen=True):
        ...
        if print_to_screen:
            logger.info(f"Rwp: {self.Rwp * 100.0:.2f} % and chi^2: {self.gofF:.2f}")
            logger.info(
                f"Rwpb: {self.Rwpb * 100.0:.2f} % and chi_b^2: {self.gofFb:.2f}\n"
            )
    ...
    def compute_tth_after_shifts(self, p, k):
        ...
        if self.phases[p][k].sf_alpha is None:
            ...
            Xs = np.degrees(
                0.9
                * (1.5 * alpha + beta)
                * (self.sf_lfactor[p][k] * lam / self.phases[p][k].lparms[0])
            )
        tth = self.tth[p][k] + self.zero_error + shft_c + trns_c + sf_shift
    ...
    def compute_texture_data(
        ...
        for mat_key, model in self.texture_model.items():
            ...
            for ii, nnz in enumerate(results[3]):
                ...
                nn = np.min((t.shape[0], ints_comp.shape[0], ints.shape[0]))
                for jj in range(nn):
