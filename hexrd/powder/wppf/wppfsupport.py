...
def _add_lp_to_params(params, mat):
    ...
    for n, l in zip(name, lp):
        ...
        """
        if n in ['a', 'b', 'c']:
            params.add(nn, value=l, min=l - 0.025, max=l + 0.025, vary=False)
        else:
...
def _generate_default_parameters_amorphous_model(params, amorphous_model):
    '''
...
def _generate_default_parameters_LeBail(
    mat, peakshape, bkgmethod, init_val=None, amorphous_model=None
):
    """
    ...
    _generate_default_parameters_amorphous_model(params, amorphous_model)
...
def _add_texture_model_parameters(texture_model, params):
    if texture_model is not None:
        for k, hm in texture_model.items():
            if hm is not None:
                hm.get_parameters(params, vary=False)


def _generate_default_parameters_Rietveld(
    mat, peakshape, bkgmethod, init_val=None, amorphous_model=None, texture_model=None
):
    """
    @author:  Saransh Singh, Lawrence Livermore National Lab
    @date:    03/12/2021 SS 1.0 original
    ...
    params = _generate_default_parameters_LeBail(
        mat, peakshape, bkgmethod, init_val, amorphous_model=amorphous_model
    )
...
_rqd_shkl = {
    ...
    "hexagonal": [
        (0, 2, 4),
        ((0, 1, 1.0), (0, 6, 2.0), (0, 9, 2.0), (0, 3, 3.0), (4, 5, 1.0), (4, 14, 1.0)),
    ],
    "trigonal": [
        (0, 2, 4, 10),
        (
