...
class DeLaValleePoussinKernel(SO3Kernel):
    ...
    def eval(
        ...
        """
        if self.has_symmetry:
            return self._symmetrized_eval(R1, R2)
        return self._kernel_values(self.misorientation_angle(R1, R2))

    def _kernel_values(self, omega: _AngleResult) -> _AngleResult:
        """Raw kernel K(ω) = C · cos(ω/2)^(2κ) from a misorientation angle."""
        co2 = np.cos(omega / 2.0)
        return self._C * co2 ** (2.0 * self._kappa)

    def _symmetrized_eval(
        self, R1: np.ndarray, R2: np.ndarray
    ) -> _AngleResult:
        """
        Symmetry-aware kernel: the AVERAGE of the kernel over all symmetry
        images of the reference orientation,

            K_sym(g, g0) = (1 / |G|) · Σ_{h ∈ G} K(ω(g, h · g0)),

        where G is the group generated by the sample (left) and crystal
        (right) symmetry operations. Averaging keeps the kernel a valid
        MRD density: each term has mean 1 over SO(3), so the average does
        too. (Using the single smallest / disorientation angle instead -
        the previous behavior - made the kernel ≈ |G| times too large, so
        symmetric ODFs were not normalized to mean 1.)
        """
        # Same contract as the reduced-angle path: one operand is a single
        # reference orientation, the other is the (broadcast) query batch.
        n1 = int(np.prod(R1.shape[:-2], dtype=int))
        n2 = int(np.prod(R2.shape[:-2], dtype=int))
        if n1 == 1:
            ref, batch = R1.reshape(3, 3), np.asarray(R2)
        elif n2 == 1:
            ref, batch = R2.reshape(3, 3), np.asarray(R1)
        else:
            raise ValueError(
                "symmetry-aware evaluation requires one operand to be a "
                "single orientation of shape (3, 3); got batches of shape "
                f"{R1.shape} and {R2.shape}. Evaluate a single reference "
                "orientation against a batch."
            )
        ...
        for s_op in sample_ops:
            for c_op in crystal_ops:
                # Symmetry image of the reference: sample on the left,
                # crystal on the right.
                equiv = s_op @ ref @ c_op
                trace = np.trace(np.matmul(batch_T, equiv), axis1=-2, axis2=-1)
                omega = np.arccos(np.clip((trace - 1.0) / 2.0, -1.0, 1.0))
                total = total + self._kernel_values(omega)

        result = total / (sample_ops.shape[0] * crystal_ops.shape[0])
        if result.ndim == 0:
            return float(result)
        return result
