...
static PyTypeObject SgOpsType;
static PyTypeObject EqMIxType;

#define is_SgOpsObject(v)  (Py_TYPE(v) == (void *) &SgOpsType)
#define is_EqMIxObject(v)  (Py_TYPE(v) == (void *) &EqMIxType)
...
static PyObject *w_get_ss(PyObject *self, PyObject *args)
{
  ...
  rangei(n_ssVM) {
    ...
    v = PyLong_FromLong((long) ssVM[i].M);
    if (! v) goto finally;
...
static PyTypeObject SgOpsType = {
        ...
        0,                                /* tp_traverse */
        0,                                /* tp_clear */
        (richcmpfunc)CMP_SgOpsObject,     /* tp_richcompare */
        0,                                /* tp_weaklistoffset */
        0,                                /* tp_iter */
        0,                                /* tp_iternext */
        SgOps_methods                     /* tp_methods */
};
...
static struct PyModuleDef moduledef = {
    ...
    "sglite",     /* m_name */
    "sglite - space group library",  /* m_doc */
    -1,                  /* m_size */
...
//DL_EXPORT(void)
PyMODINIT_FUNC PyInit_sglite(void)
{
  PyObject  *m, *d, *s;
  ...
  s = PyUnicode_FromStringAndSize(Revision, strlen(Revision ) );
  ...
  ErrorObject = PyUnicode_FromString("sglite.error");
