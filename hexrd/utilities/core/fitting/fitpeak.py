...
from hexrd.utilities.core.imageutil import snip1d
from hexrd.utilities.core.fitting import peakfunctions as pkfuncs
...
def fit_pk_parms_1d(p0, x, f, pktype='pvoigt'):
    ...
    weight = np.max(f) * 10.0  # hard coded should be changed
    fitArgs = (x, f, pktype)
    if pktype == 'gaussian':
        ...
        lb = [p0[0] * 0.5, np.min(x), 0.0, 0.0, 0.0, None]
        ub = [p0[0] * 2.0, np.max(x), 4.0 * p0[2], 1.0, 2.0 * p0[4], None]

        fitArgs = (x, f, pktype, weight, lb, ub)
        p, outflag = optimize.leastsq(
            fit_pk_obj_1d_bnded, p0, args=fitArgs, ftol=ftol, xtol=xtol
        )
    elif pktype == 'split_pvoigt':
        # fmt: off
        ...
        p, outflag = optimize.leastsq(
            fit_pk_obj_1d, p0, args=fitArgs, ftol=ftol, xtol=xtol
        )
...
def fit_mpk_parms_1d(p0, x, f0, pktype, num_pks, bgtype=None, bnds=None):
    ...
    if bnds is None:
        p = optimize.least_squares(
            fit_mpk_obj_1d, p0, args=fitArgs, ftol=ftol, xtol=xtol
        )
    else:
        p = optimize.least_squares(
            fit_mpk_obj_1d, p0, bounds=bnds, args=fitArgs, ftol=ftol, xtol=xtol
        )
    return p.x
...
def estimate_mpk_parms_1d(
    ...
    fwhm_guess=None,
    center_bnd=0.02,
    amp_lim_mult=[0.1, 10.0],
    fwhm_lim_mult=[0.5, 2.0],
):
    """
    ...
    if pktype == 'gaussian' or pktype == 'lorentzian':
        ...
        for ii in np.arange(num_pks):
            ...
            p0tmp_ub[ii, :] = [
                (amp_guess - min_val + 1.0) * amp_lim_mult[1],
                pk_pos_0[ii] + center_bnd[ii],
                fwhm_guess[ii] * fwhm_lim_mult[1],
                1.0,
            ]
...
def estimate_pk_parms_2d(x, y, f, pktype):
    ...
    if pktype == 'gaussian':
        ...
        p = [A, x0, y0, FWHMx, FWHMy, 0.0, bg0, bg1x, bg1y]
...
def direct_pk_analysis(
    ...
    if total_int < low_int:
        ...
        a = np.abs(ffine[cen_index + 1 :] - A / 2.0)
        b = np.abs(ffine[:cen_index] - A / 2.0)
