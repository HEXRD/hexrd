...
from hexrd.utilities.core.constants import (
    c_erf,
    cnum_exp1exp,
    cden_exp1exp,
    c_coeff_exp1exp,
)
from hexrd.utilities.core.matrixutil import uniqueVectors
...
def _set_equality_constraints(params, pname_spec):
    if isinstance(pname_spec, str):
        ...
        for name_pair in pname_spec:
            assert len(name_pair) == 2, "entries in name spec must be 2-tuples"
            params[name_pair[0]].expr = name_pair[1]


def _set_bound_constraints(
    params,
    pname_spec,
    min_val=-np.inf,
    max_val=np.inf,
    ...
    for pname in target_pnames:
        if box is None:
            ...
            hval = 0.5 * box
            if percentage:
                hval = 0.5 * abs(params[pname].value * (box / 100.0))
