...
def gather_confidence(controller, confidence, n_grains, n_coords):
    ...
    send_counts[-1] = (n_coords - (coords_per_rank *
                       (world_size-1))) * n_grains
    ...
    comm.Gatherv(confidence.T.flatten(),
                 (global_confidence, send_counts), root=0)
    if rank == 0:
...
def gen_nf_test_grid(cross_sectional_dim, v_bnds, voxel_spacing):
    ...
    Zs_list = np.arange(-cross_sectional_dim/2.+voxel_spacing /
                        2., cross_sectional_dim/2., voxel_spacing)
    Xs_list = np.arange(-cross_sectional_dim/2.+voxel_spacing /
                        2., cross_sectional_dim/2., voxel_spacing)

    if v_bnds[0] == v_bnds[1]:
        Xs, Ys, Zs = np.meshgrid(Xs_list, v_bnds[0], Zs_list)
    else:
        Xs, Ys, Zs = np.meshgrid(Xs_list, np.arange(
            v_bnds[0]+voxel_spacing/2., v_bnds[1], voxel_spacing), Zs_list)
...
def gen_nf_test_grid_tomo(x_dim_pnts, z_dim_pnts, v_bnds, voxel_spacing):
    ...
    if v_bnds[0] == v_bnds[1]:
        ...
        Xs, Ys, Zs = np.meshgrid(np.arange(x_dim_pnts), np.arange(
            v_bnds[0]+voxel_spacing/2., v_bnds[1], voxel_spacing), np.arange(z_dim_pnts))
        #note numpy shaping of arrays is goofy, returns(length(y),length(x),length(z))
    ...
    if process_type == 'gaussian':
        ...
        for ii in np.arange(img_nums.shape[0]):
            ...
            tmp_img = imgio.imread(
                data_folder+'%s' % (stem)+str(img_nums[ii]).zfill(num_digits)+ext)-dark
            #image procesing
        ...
        for ii in np.arange(img_nums.shape[0]):
            ...
            tmp_img = imgio.imread(
                data_folder+'%s' % (stem)+str(img_nums[ii]).zfill(num_digits)+ext)-dark
            #image procesing
            image_stack[ii, :, :] = img.morphology.binary_erosion(
                tmp_img > threshold, iterations=num_erosions)
            image_stack[ii, :, :] = img.morphology.binary_dilation(
                image_stack[ii, :, :], iterations=num_dilations)
...
def gen_trial_exp_data(grain_out_file, det_file, mat_file, mat_name, max_tth, 
                       ome_range_deg, nframes, beam_stop_parms, 
                       comp_thresh=0.0, chi2_thresh=1.0, misorientation_bnd=None,
    ...
    cut = np.where(np.logical_and(
        completeness > comp_thresh, chi2 < chi2_thresh))[0]
    ...
    ome_period_deg = (ome_range_deg[0][0],
                      (ome_range_deg[0][0]+360.))  # degrees
...
def save_raw_confidence(save_dir, save_stem, raw_confidence, id_remap=None):
    ...
    if id_remap is not None:
        np.savez(save_dir+save_stem+'_raw_confidence.npz',
                 raw_confidence=raw_confidence, id_remap=id_remap)
    else:
        np.savez(save_dir+save_stem+'_raw_confidence.npz',
...
def save_nf_data(save_dir, save_stem, grain_map, confidence_map, Xs, Ys, Zs, ori_list, id_remap=None):
    ...
    if id_remap is not None:
        np.savez(save_dir+save_stem+'_grain_map_data.npz', grain_map=grain_map,
                 confidence_map=confidence_map, Xs=Xs, Ys=Ys, Zs=Zs, ori_list=ori_list, id_remap=id_remap)
    else:
        np.savez(save_dir+save_stem+'_grain_map_data.npz', grain_map=grain_map,
                 confidence_map=confidence_map, Xs=Xs, Ys=Ys, Zs=Zs, ori_list=ori_list)
...
def scan_detector_parm(image_stack, experiment, test_crds, controller, parm_to_opt, parm_range, slice_shape, ang='deg'):
    ...
    parm_vector = np.arange(
        parm_range[0], parm_range[1]+1e-6, (parm_range[1]-parm_range[0])/parm_range[2])
    ...
    for jj in np.arange(num_parm_pts):
        ...
        if parm_to_opt == 6:
            ...
            experiment.ome_range = [
                (ome_range[0][0]-parm_vector[jj], ome_range[0][1]-parm_vector[jj])]
            experiment.ome_period = (
                ome_period[0]-parm_vector[jj], ome_period[1]-parm_vector[jj])
...
def output_grain_map(data_location, data_stems, output_stem, vol_spacing, top_down=True, save_type=['npz']):
    ...
    for ii in np.arange(num_scans):
        ...
        conf_data = np.load(os.path.join(
            data_location, data_stems[ii]+'_grain_map_data.npz'))
    ...
    for ii in np.arange(num_scans):
        if top_down == True:
            grain_map_stitched[((ii)*num_layers):((ii)*num_layers +
                                                  num_layers), :, :] = grain_maps[num_scans-1-ii]
            confidence_stitched[((ii)*num_layers):((ii)*num_layers +
                                                   num_layers), :, :] = confidence_maps[num_scans-1-ii]
            Xs_stitched[((ii)*num_layers):((ii)*num_layers+num_layers), :, :] =\
                Xss[num_scans-1-ii]
            Zs_stitched[((ii)*num_layers):((ii)*num_layers+num_layers), :, :] =\
                Zss[num_scans-1-ii]
            Ys_stitched[((ii)*num_layers):((ii)*num_layers+num_layers),
                        :, :] = Yss[num_scans-1-ii]+vol_shifts[ii]
        else:

            grain_map_stitched[((ii)*num_layers):((ii) *
                                                  num_layers+num_layers), :, :] = grain_maps[ii]
            confidence_stitched[(
                (ii)*num_layers):((ii)*num_layers+num_layers), :, :] = confidence_maps[ii]
            Xs_stitched[((ii)*num_layers):((ii)*num_layers +
                                           num_layers), :, :] = Xss[ii]
            Zs_stitched[((ii)*num_layers):((ii)*num_layers +
                                           num_layers), :, :] = Zss[ii]
            Ys_stitched[((ii)*num_layers):((ii)*num_layers +
                                           num_layers), :, :] = Yss[ii]+vol_shifts[ii]
