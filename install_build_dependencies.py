...
# Define where to place the libraries
...
def download_xsimd(path):
  url = 'https://github.com/xtensor-stack/xsimd/archive/refs/tags/7.6.0.tar.gz'
  ...
  with tempfile.TemporaryDirectory() as temp_dir:
    ...
    os.makedirs(path, exist_ok=True)
    shutil.move(str(Path(temp_dir) / out_dir_name / 'include/xsimd'), str(Path(path) / 'xsimd/xsimd'))
