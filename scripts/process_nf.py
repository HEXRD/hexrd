...
main_dir = '/nfs/chess/user/relim/pagan-3274-a/ti7al_bruker'
# main_dir = "/Users/rachellim/Documents/PSU/CHESS_2022Feb/mono_study"
...
output_dir = '/nfs/chess/user/relim/NF_chunking'
...
grain_out_file = main_dir + '/ti7al-6_grains_best.out'
# grain_out_file = '/Users/rachellim/Documents/PSU/CHESS_2022Feb/mono_study/ti7al-6_grains.out'


#%%

#Locations of near field images
data_folder = '/nfs/chess/raw/2022-1/id3a/pagan-3274-a/ti7al-5/1/nf'

img_start = 106188  # whatever you want the first frame to be
nframes = 1440
img_nums = np.arange(img_start, img_start+nframes, 1)

output_stem = '/ti7al-6_nf_scan9'
#corresponding scan number
#==============================================================================
...
misorientation_spacing = 0.2  # degrees
...
ome_range_deg = [(-0.55, 359.2)]  # degrees
...
voxel_spacing = 0.002  # in mm, voxel spacing for the near field reconstruction


##vertical (y) reconstruction voxel bounds in mm, ALWAYS USED REGARDLESS OF MASK
#v_bnds=[-0.0025,0.0025] #(mm) if bounds are equal, a single layer is produced
# v_bnds = [-0.085,0.085]
v_bnds = [0, 0]
...
print(n_groups)
print(leftover_voxels)

grouped_voxels = n_voxels - leftover_voxels

voxels_per_group = grouped_voxels/n_groups  # np.floor(n_voxels/(n_groups+1))
...
image_stack_load = np.load(main_dir + '/ti7al-6_cleaned_nf_scan9_thresh30.npz')


image_stack = np.array(image_stack_load['arr_0'])
# #%%
# plt.imshow(image_stack[10,:,:])
# plt.colorbar()

# plt.figure()
# plt.imshow(image_stack[30,:,:])
# plt.colorbar()
...
a = plt.figure()
plt.imshow(grain_map[0], cmap='jet')

b = plt.figure()
plt.imshow(confidence_map[0], vmin=0.45, vmax=1)
plt.colorbar()
#==============================================================================
...
# np.savez_compressed(output_dir + output_stem + '_raw_conf.npz', raw_confidence_full)
